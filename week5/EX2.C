    ...
    if (*(char*)pdata == '4') {//decision task일 경우
        ...
        for (;;) {
            ...
            if (*(char*)pdata == '0') {
                ...
                OSFlagPend(r_grp, 0x01, OS_FLAG_WAIT_SET_ALL + OS_FLAG_CONSUME, 0, &err);
                bgnd_color = DISP_BGND_RED;
                fgnd_color = DISP_FGND_RED;
            }
            else if (*(char*)pdata == '1') {
                OSFlagPost(s_grp, 0x02, OS_FLAG_SET, &err);
                OSFlagPend(r_grp, 0x02, OS_FLAG_WAIT_SET_ALL + OS_FLAG_CONSUME, 0, &err);
                bgnd_color = DISP_BGND_CYAN;
                fgnd_color = DISP_FGND_CYAN;
            }
            else if (*(char*)pdata == '2') {
                OSFlagPost(s_grp, 0x04, OS_FLAG_SET, &err);
                OSFlagPend(r_grp, 0x04, OS_FLAG_WAIT_SET_ALL + OS_FLAG_CONSUME, 0, &err);
                bgnd_color = DISP_BGND_BLUE;
                fgnd_color = DISP_FGND_BLUE;
            }
            else if (*(char*)pdata == '3') {
                OSFlagPost(s_grp, 0x08, OS_FLAG_SET, &err);
                OSFlagPend(r_grp, 0x08, OS_FLAG_WAIT_SET_ALL + OS_FLAG_CONSUME, 0, &err);
            ...
            PC_DispStr(8 + 18 * task_number, 4, "[", DISP_FGND_WHITE + bgnd_color);
            PC_DispStr(10 + 18 * task_number, 4, "]", DISP_FGND_WHITE + bgnd_color);
