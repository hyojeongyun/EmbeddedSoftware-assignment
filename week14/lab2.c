...
int main (void)
{
  ...
  OSTaskCreate(BuzzerTask, (void *)0, (void *)&TaskStk[2][TASK_STK_SIZE - 1], 2);
...
void FndTask (void *data)
{
	...
	while (1) {
		...
		OSFlagPost(e_grp, 0x03, OS_FLAG_SET, &err);
...
void BuzzerTask(void *data)
{
	...
	OSFlagPend(e_grp, 0x02, OS_FLAG_WAIT_SET_ALL + OS_FLAG_CONSUME, 0, &err);
